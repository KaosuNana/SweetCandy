...
namespace SweetSugar.Scripts.Editor
{
    ...
    public class LevelMakerEditor : EditorWindow
    {
        ...
        private void OnFocus()
        {
            ...
            targetEditorScriptable = AssetDatabase.LoadAssetAtPath<TargetEditorScriptable>("Assets/SweetSugar/Resources/Levels/TargetEditorScriptable.asset");
        ...
        private void Initialize()
        {
            ...
            squareTypeGroupped = _squareTypeItems.Select((x, i) => new {Index = i, Value = x}).GroupBy(i => i.Index / 6).Select(i => i.Select(x => x.Value))
                .ToArray();
...
//            GooglePlayServices.PlayServicesResolver.MenuResolve();//2.1.2
        ...
        private void GUIAds()
        {
            ...
            unityAdsSettings.androidID = EditorGUILayout.TextField("Android ID ", unityAdsSettings.androidID, GUILayout.Width (220));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Space(20);
            ...
            GUILayout.Space(20);
            adsSettings.admobRewardedUIDAndroid = EditorGUILayout.TextField("Admob Rewarded ID Android ", adsSettings.admobRewardedUIDAndroid, new GUILayoutOption[] {
                GUILayout.Width (220),
                GUILayout.MaxWidth (220)
            });
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Space(20);
            adsSettings.admobRewardedUIDIOS = EditorGUILayout.TextField("Admob Rewarded ID iOS", adsSettings.admobRewardedUIDIOS, new GUILayoutOption[] {
                GUILayout.Width (220),
                GUILayout.MaxWidth (220)
            });
            GUILayout.EndHorizontal();
...
//        }
        ...
        {
            GUILayout.Label("Sweet Candy 1.0", EditorStyles.boldLabel,
                GUILayout.Width(400));
            ...
            GUILayout.Space(10);
            GUILayout.TextArea("melogigame@gmail.com", EditorStyles.boldLabel, GUILayout.Width(350));
        }
...
//        GUILayout.Space(20);
            ...
            EditorGUI.BeginChangeCheck();
            winReward.winRewardAmount = EditorGUILayout.IntField("Win reward gems", winReward.winRewardAmount, GUILayout.Width(200),
                GUILayout.MaxWidth(200));
            if (EditorGUI.EndChangeCheck())
            {
...
//        GUILayout.Space(35);
        ...
        private void GUITarget()
        {
                        ...
                        if (targetObjects != null)
                        {
                            ...
                            if(target.setCount == SetCount.Manually)
                            {
                                foreach (var targetObject in targetObjects)
                                {
                                    GUILayout.BeginHorizontal();
                                    {
                                        EditorGUILayout.LabelField(targetObject.sprites.FirstOrDefault().icon.name, GUILayout.Width(50));
                                        EditorGUI.BeginChangeCheck();
                                        targetObject.CountDrawer.count = EditorGUILayout.IntField(targetObject.CountDrawer.count, GUILayout.Width(50));
                                        if(EditorGUI.EndChangeCheck())
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                GUILayout.Space(103);
                int newselectedTutorial = EditorGUILayout.Popup(levelData.selectedTutorial, GetTutorialNames(),GUILayout.Width(160),GUILayout.ExpandWidth (false));
                            ...
                            foreach (var val in itemsForEditor)
                            {
                                GUILayout.BeginHorizontal();
                                {
                                    foreach (var item in val)
                                    {
                                        ...
                                        if(itemBrush != null && (itemBrush.ItemType == item.ItemType && item.ItemType == ItemsTypes.NONE && color == item.Color && itemBrush
                                        .Texture == item.Texture))
                                            UnityEngine.GUI.backgroundColor = Color.gray;
                                        else if(itemBrush != null && (itemBrush.ItemType == item.ItemType && item.ItemType != ItemsTypes.NONE && itemBrush
                                            .Texture == item.Texture))
                                            UnityEngine.GUI.backgroundColor = Color.gray;
...
//                                if (HideAnotherTargetBlock(squareTypeItem.ToString())) continue;
                ...
                for (int col = 0; col < GetField().maxCols; col++)
                {
                    ...
                    if (section == 0 || section == 1)
                    {
                        if (squareBlock.block != SquareTypes.NONE )
                        {
                            ...
                            if(squareBlock.item != null && squareBlock.item.Item != null && squareBlock.item?.ItemType != ItemsTypes.INGREDIENT)
                                blockTexturesArray.Add(new Texture2DSize(squareBlock.item?.Item.GetComponent<IColorableComponent>().GetSprite(levelNumber, squareBlock.item
                            ...
                            else 
                            {
                                    ...
                                    if(!blockTexturesArray.Any(i=>i.Texture2D.name == t.Texture2D.name))
                                        blockTexturesArray.Insert(0, new Texture2DSize(t.Texture2D, Vector2Int.one, t.order, rotateBig));
                                    if(squareBlock.item?.Texture != null)
                                        blockTexturesArray.Add(new Texture2DSize(squareBlock.item?.Texture, Vector2Int.one, squareBlock.item.order, rotateBig));
                            }
                        }
                        if (squareBlock.item?.ItemType == ItemsTypes.PACKAGE)
                        {
                            ...
                            var texture2 = squareBlock.item?.Item.GetComponent<IColorableComponent>().GetSprite(levelNumber, squareBlock.item.Color).texture;
                            if(texture1.width == texture2.width && texture1.height == texture2.height)
                                blockTexturesArray.Add(new Texture2DSize(texture1?.AlphaBlend(texture2),Vector2Int.one, 1000, rotateBig));
                        }
                    }
        ...
        private void DrawTexture( Texture2DSize texture2DSize, Rect lastRect)
        {
            ...
            if (texture2D != null)
            {
                var rect = new Rect(lastRect.position.x + 4, lastRect.position.y + 4, texture2DSize.size.x * 40, texture2DSize.size.y * 40);
                if (texture2DSize.size.magnitude >= 2 && texture2DSize.rotate && texture2D.width < 2048 )
                {
                    texture2D = texture2D.rotateTexture();
                    rect = new Rect(lastRect.position.x + 4, lastRect.position.y + 4, texture2DSize.size.y * 40, texture2DSize.size.x * 40);
                }
        ...
        {
            SquareBlocks squareBlock = levelData.GetBlock(row, col);
            if(squareBlock.blocks.Any(i=>i.squareType == SquareTypes.SolidBlock||i.squareType==SquareTypes.UndestrBlock||i.squareType==SquareTypes.NONE)) return;
        ...
        private void SetSquareType(int col, int row)
        {
            ...
            else
            {
                ...
                if(!deleteBlock)
                {
                    if (!squarePrototype.IsObstacle() && squareType != SquareTypes.EmptySquare && squareType != SquareTypes.NONE)
                    {
                        if (squareBlock.block == squareType)
                            squareBlock.blockLayer = (int) Mathf.Repeat(squareBlock.blockLayer, Square.GetLayersCount(squareType)) + 1;
                        else
                            squareBlock.blockLayer = 1;
                        squareBlock.block = squareType;
                    }
                    else if (squarePrototype.IsObstacle())
                    {
                        if (squareBlock.obstacle == squareType)
                            squareBlock.obstacleLayer = (int) Mathf.Repeat(squareBlock.obstacleLayer, Square.GetLayersCount(squareType)) + 1;
                        else
                            squareBlock.obstacleLayer = 1;
